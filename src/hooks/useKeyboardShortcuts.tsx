...
export const useKeyboardShortcuts = ({
  ...
  useEffect(() => {
    const actions: Record<string, KeybindHandler> = {
      ...
      toggle_rejected: {
        shouldFire: () => multiSelectedPaths.length > 0 || !!selectedImage,
        execute: (event) => { event.preventDefault(); handleToggleRejected(); },
      },
      preview_prev: {
    ...
    const builtinShortcuts: Array<{ match: BuiltInMatch; execute: BuiltInExec }> = [
      ...
      {
        match: (e) => !selectedImage && ['Enter', 'Space'].includes(e.code),
        execute: (e) => {
          e.preventDefault();
          const activePath = libraryActivePath;
